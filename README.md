...
directory does a lot of that for you. You would run it like so:
    ...
    go get github.com/missmp/oapi-codegen/cmd/oapi-codegen
...
When generate with -include-tags, we will use the first tag for ServerInterface prefix. 

```go
type {{Prefix}}ServerInterface interface {
    //  (GET /pets)
...
When generate with -include-tags, we will use the first tag for both RegisterHandlers and ServerInterfaceWrapper prefix. 
 
For the pet store, it looks like this:
```go
func {{Prefix}}RegisterHandlers(router codegen.EchoRouter, si ServerInterface) {
    wrapper := {{Prefix}}ServerInterfaceWrapper{
        Handler: si,
    }
...
```
    import (
        "github.com/missmp/oapi-codegen/pkg/securityprovider"
    )
